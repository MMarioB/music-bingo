...
export function createGameServer({ snapshotPath = null } = {}) {
      ...
      try {
        ...
        if (config.roomCode !== undefined) {
          ...
          if (gameRooms.has(roomCode) || orphanedRooms.has(roomCode)) {
            const existing = gameRooms.get(roomCode);
            // Reintento benigno del mismo host (p. ej. doble efecto de React):
            // devolver la sala existente en vez de error
  ...
  // Catálogo de previews con caché en memoria: el cliente pide las
  // canciones de un artista y recibe MP3s de 30s reproducibles sin Spotify
